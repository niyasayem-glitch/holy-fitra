...
class RaggedBatch:
    ...
    def validate(self) -> None:
        ...
        if self.q.shape[1] != self.d_model or self.offsets.dtype.kind not in "iu":
            raise RaggedAttentionError("invalid dimension or offsets dtype")
...
def pack_sequences(sequences: Iterable[tuple[np.ndarray, np.ndarray, np.ndarray]]) -> RaggedBatch:
    ...
    for q, k, v in rows:
        if q.ndim != 2 or k.shape != q.shape or v.shape != q.shape or q.shape[1] != d_model or q.shape[0] <= 0:
    ...
    lengths = np.asarray([row[0].shape[0] for row in rows], dtype=np.int32)
...
def ragged_work(batch: RaggedBatch) -> int:
    batch.validate()
    lengths = np.diff(batch.offsets.astype(np.int64))
    return int(np.sum(lengths * lengths) * batch.d_model)
