...
## State-9 persistent child arenas and imported-call validation — 2026-08-22
State 9 preserves parser child relationships in global bounded parallel arenas (`child_modules`, `child_parents`, and `child_nodes`) with per-module base/count metadata, derives export return types and parameter counts/types from parser-produced function nodes, and validates imported call nodes before HIR lowering. The parser now accepts typed parameters, public function markers, integer and boolean arguments, and comma-separated call argument lists. The positive `app -> core.inc(41)` call is accepted with return type ID 1; `bad_arity -> core.inc()` emits HF5001 and `bad_type -> core.inc(true)` emits HF5002. Unsupported multi-import call binding remains fail-closed rather than being guessed.

The no-Python bootstrap gate passed States 1–9, including repeated State-9 AST/signature/HIR/child snapshots, State-9 ASAN/UBSan execution with leak detection, and AArch64 Android-target object emission. The State-9 AArch64 object produced by the sandbox gate was 89,392 bytes; this is a cross-compilation artifact only and is not evidence of physical Android execution. The complete matrix also passed 153 Python tests, Python compile checks, shell syntax checks, runtime sanitizer checks, invalid-input diagnostics, and the Termux-compatible host gate. CFG/MIR lowering, a multi-import call environment, and full fixed-point self-hosting remain future work.
