...
| Native AI runtime | NibbleFlow INT4 kernels with opt-in static-INT8 activation execution, calibration quality gates, bounded low-rank residual adapters, authenticated capsule adapter-residency lanes with rollback receipts, deterministic KV residency and precision-governor contracts, ragged kernels, bounded parallel matvec micro-batches behind one cancellable request, portable four-output tiles, compatible four-row packed-weight reuse, local per-batch range receipts, JNI wrappers, and versioned benchmark receipts | Host/native/sanitizer gates, deterministic residency stress and capsule-tamper checks, host-only micro-batch and large-model-versus-Python/OpenBLAS benchmark evidence, AArch64 object emission when an NDK sysroot is available, and remote Android arm64-v8a package verification; device performance remains unmeasured |
