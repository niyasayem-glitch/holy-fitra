...
| AI integration | Provider-neutral chat, embeddings, validated Holy Fitra generation, model selection, request boundaries, credential-safe provider status, deterministic local UTF-8-byte causal bigram and bounded sparse n-gram baselines, plus a bounded trainable single-head embedding-attention baseline with checkpoint and evaluation receipts | Unit tests and explicit provider configuration; the local baselines provide one- or bounded two-to-four-token context, or one bounded 16-token single-head attention layer. They are not transformer-scale systems, Qwen comparisons, general language models, coding models, or multimodal models |
| Coding automation | Plan-first supervised agent, workspace confinement, allowlisted commands, transactional writes, rollback, improvement rounds, high-risk branch restrictions, and HD read-only coding advice, bounded Obsidian-compatible provenance retrieval, file-by-file unified diff previews, and explicitly approved foreground campaigns of at most three cycles | Unit tests and campaign dry-run gates; HD does not gain unrestricted control, background execution, external Obsidian synchronization, arbitrary shell/network access, automatic apply authority, or an indefinite loop |
