...
| Native AI runtime | NibbleFlow INT4 kernels with opt-in static-INT8 activation execution, calibration quality gates, bounded low-rank residual adapters, authenticated capsule adapter-residency lanes with rollback receipts, deterministic KV residency and precision-governor contracts, ragged kernels, bounded parallel matvec micro-batches behind one cancellable request, local per-batch range receipts, JNI wrappers, and versioned benchmark receipts | Host/native/sanitizer gates, deterministic residency stress and capsule-tamper checks, host-only micro-batch and large-model-versus-pure-Python benchmarks, AArch64 object emission, and remote Android arm64-v8a package verification; device performance remains unmeasured |
