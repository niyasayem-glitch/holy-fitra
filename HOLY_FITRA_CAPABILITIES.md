...
| AI integration | Provider-neutral chat, embeddings, validated Holy Fitra generation, model selection, request boundaries, credential-safe provider status, and a deterministic local UTF-8-byte causal-bigram baseline with checkpoint and evaluation receipts | Unit tests and explicit provider configuration; the local baseline is a one-token-context bigram prerequisite, not a transformer, Qwen comparison, general language model, coding model, or multimodal model |
