...
The v1 driver is intentionally scoped to the verified scalar compiler subset and uses the locally installed `clang` toolchain. On an ARM64 Termux device it automatically selects `aarch64-linux-android` and lets the Termux clang driver use its native Bionic environment; on x86-64 Linux it retains the x86-64 host target. Set `HOLYFITRA_TARGET` to request an explicit target, and set `HOLYFITRA_CC`/`HOLYFITRA_CXX` to select compilers. `HOLYFITRA_SEED_OPT` controls the bootstrap compiler optimization level and defaults to `O0` for faster cold startup; use `O2` or `O3` when seed execution throughput matters more. A foreign `--target` produces an artifact only and does not prove execution on that platform.
