...
int main() {
    ...
    const hf_runtime_stats before_fused = hf_runtime_get_stats(runtime);
    ...
    const hf_runtime_stats after_fused = hf_runtime_get_stats(runtime);
    std::printf("batch=%zu single_ms=%.3f fused_ms=%.3f speedup=%.2fx fused_tasks=%llu\n", batch, single_seconds * 1000.0, batch_seconds * 1000.0, single_seconds / batch_seconds, static_cast<unsigned long long>(after_fused.completed - before_fused.completed));
