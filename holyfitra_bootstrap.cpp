...
struct Type final {
    ...
    std::string llvm() const {
        switch (kind) {
            ...
            case TypeKind::Handle: return "i8*";
            case TypeKind::File: return "i8*";
            case TypeKind::Buffer: return "i8*";
            case TypeKind::Array: return "[" + std::to_string(count) + " x " + element->llvm() + "]";
            case TypeKind::DynamicArray: return "i8*";
...
private:
    ...
    std::pair<std::string,Type> expr(const Expr&e,std::ostringstream&o,std::optional<Type> expected=std::nullopt){
        ...
        if(e.kind==Expr::Kind::String){auto&s=static_cast<const StringExpr&>(e).value;auto id=strings_.at(s);auto r=tmp();o<<"  "<<r<<" = getelementptr inbounds ["<<s.size()+1<<" x i8], ["<<s.size()+1<<" x i8]* @.str."<<id<<", i64 0, i64 0\n";return {r,Type::scalar(TypeKind::String)};}
        if(e.kind==Expr::Kind::Name){auto&n=static_cast<const NameExpr&>(e).name;auto&l=locals_.at(n);auto r=tmp();o<<"  "<<r<<" = load "<<l.type.llvm()<<", "<<l.type.llvm()<<"* "<<l.addr<<"\n";return {r,l.type};}
        ...
        if(e.kind==Expr::Kind::Index){auto&x=static_cast<const IndexExpr&>(e);Type base=localType(*x.base);if(base.kind==TypeKind::DynamicArray&&base.element->kind==TypeKind::I32){auto a=expr(*x.base,o);auto i=expr(*x.index,o,Type::scalar(TypeKind::I64));auto r=tmp();o<<"  "<<r<<" = call i32 @hf_dyn_i32_get(i8* "<<a.first<<", i64 "<<i.first<<")\n";return {r,Type::scalar(TypeKind::I32)};}}
        if(e.kind==Expr::Kind::Field||e.kind==Expr::Kind::Index){auto a=address(e,o);auto r=tmp();Type t=localType(e);o<<"  "<<r<<" = load "<<t.llvm()<<", "<<t.llvm()<<"* "<<a.first<<"\n";return {r,t};}
