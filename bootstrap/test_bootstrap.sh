#!/usr/bin/env bash
set -euo pipefail

ROOT=$(cd "$(dirname "$0")/.." && pwd)
BUILD=${HOLYFITRA_BOOTSTRAP_BUILD:-/tmp/holyfitra_bootstrap_gate}
WORK=${HOLYFITRA_BOOTSTRAP_WORK:-/tmp/holyfitra_bootstrap_work}
mkdir -p "$WORK"

clang++ -std=c++17 -O2 -Wall -Wextra -pedantic "$ROOT/holyfitra_bootstrap.cpp" -o "$BUILD"

"$BUILD" "$ROOT/bootstrap/hello.hf" -o "$WORK/hello.ll"
clang -O2 "$WORK/hello.ll" -o "$WORK/hello"
set +e
"$WORK/hello"
hello_status=$?
set -e
test "$hello_status" -eq 42

"$BUILD" "$ROOT/bootstrap/control.hf" -o "$WORK/control.ll"
clang -O2 "$WORK/control.ll" -o "$WORK/control"
set +e
"$WORK/control"
control_status=$?
set -e
test "$control_status" -eq 1

"$BUILD" "$ROOT/bootstrap/short_circuit.hf" -o "$WORK/short_circuit.ll"
grep -F 'phi i1' "$WORK/short_circuit.ll" >/dev/null
clang -O2 "$WORK/short_circuit.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/short_circuit"
(cd "$ROOT" && "$WORK/short_circuit")
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/short_circuit.hf" -o "$WORK/short_circuit.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/short_circuit.aarch64.ll" -o "$WORK/short_circuit.aarch64.o"
test -s "$WORK/short_circuit.aarch64.o"

"$BUILD" "$ROOT/bootstrap/aggregates.hf" -o "$WORK/aggregates.ll"
clang -O2 "$WORK/aggregates.ll" -o "$WORK/aggregates"
set +e
"$WORK/aggregates"
aggregate_status=$?
set -e
test "$aggregate_status" -eq 42

"$BUILD" "$ROOT/bootstrap/selfhost_frontend.hf" -o "$WORK/selfhost_frontend.ll"
clang -O2 "$WORK/selfhost_frontend.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_frontend"
set +e
(cd "$ROOT" && "$WORK/selfhost_frontend")
selfhost_frontend_status=$?
set -e
test "$selfhost_frontend_status" -eq 0
clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all \
  "$WORK/selfhost_frontend.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_frontend_san"
(cd "$ROOT" && ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/selfhost_frontend_san")
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_frontend.hf" -o "$WORK/selfhost_frontend.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_frontend.aarch64.ll" -o "$WORK/selfhost_frontend.aarch64.o"
test -s "$WORK/selfhost_frontend.aarch64.o"

"$BUILD" "$ROOT/bootstrap/io.hf" -o "$WORK/io.ll"
clang -O2 "$WORK/io.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/io"
set +e
(cd "$ROOT" && "$WORK/io")
io_status=$?
set -e
test "$io_status" -eq 2
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/io.hf" -o "$WORK/io.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/io.aarch64.ll" -o "$WORK/io.aarch64.o"
test -s "$WORK/io.aarch64.o"

"$BUILD" "$ROOT/bootstrap/buffer.hf" -o "$WORK/buffer.ll"
clang -O2 "$WORK/buffer.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/buffer"
set +e
(cd "$ROOT" && "$WORK/buffer")
buffer_status=$?
set -e
test "$buffer_status" -eq 4
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/buffer.hf" -o "$WORK/buffer.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/buffer.aarch64.ll" -o "$WORK/buffer.aarch64.o"
test -s "$WORK/buffer.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_symtable.hf" -o "$WORK/selfhost_symtable.ll"
clang -O2 "$WORK/selfhost_symtable.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_symtable"
set +e
(cd "$ROOT" && "$WORK/selfhost_symtable")
symtable_status=$?
set -e
test "$symtable_status" -eq 3
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_symtable.hf" -o "$WORK/selfhost_symtable.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_symtable.aarch64.ll" -o "$WORK/selfhost_symtable.aarch64.o"
test -s "$WORK/selfhost_symtable.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_typechecker.hf" -o "$WORK/selfhost_typechecker.ll"
clang -O2 "$WORK/selfhost_typechecker.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_typechecker"
(cd "$ROOT" && "$WORK/selfhost_typechecker")
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_typechecker.hf" -o "$WORK/selfhost_typechecker.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_typechecker.aarch64.ll" -o "$WORK/selfhost_typechecker.aarch64.o"
test -s "$WORK/selfhost_typechecker.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_emitter.hf" -o "$WORK/selfhost_emitter.ll"
clang -O2 "$WORK/selfhost_emitter.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_emitter"
(cd "$ROOT" && "$WORK/selfhost_emitter")
test -s /tmp/holyfitra_selfhost_emitted.ll
clang -O2 /tmp/holyfitra_selfhost_emitted.ll -o "$WORK/selfhost_emitted_program"
set +e
"$WORK/selfhost_emitted_program"
emitted_status=$?
set -e
test "$emitted_status" -eq 42
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_emitter.hf" -o "$WORK/selfhost_emitter.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_emitter.aarch64.ll" -o "$WORK/selfhost_emitter.aarch64.o"
test -s "$WORK/selfhost_emitter.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_state1.hf" -o "$WORK/selfhost_state1.ll"
clang -O2 "$WORK/selfhost_state1.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state1"
(cd "$ROOT" && "$WORK/selfhost_state1")
test -s /tmp/holyfitra_state1_tokens.snapshot
test -s /tmp/holyfitra_state1_ast.snapshot
cp /tmp/holyfitra_state1_tokens.snapshot "$WORK/state1_tokens.first"
cp /tmp/holyfitra_state1_ast.snapshot "$WORK/state1_ast.first"
(cd "$ROOT" && "$WORK/selfhost_state1")
cmp -s "$WORK/state1_tokens.first" /tmp/holyfitra_state1_tokens.snapshot
cmp -s "$WORK/state1_ast.first" /tmp/holyfitra_state1_ast.snapshot
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state1.hf" -o "$WORK/selfhost_state1.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state1.aarch64.ll" -o "$WORK/selfhost_state1.aarch64.o"
test -s "$WORK/selfhost_state1.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_state2.hf" -o "$WORK/selfhost_state2.ll"
clang -O2 "$WORK/selfhost_state2.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state2"
(cd "$ROOT" && "$WORK/selfhost_state2")
test -s /tmp/holyfitra_state2_ast.snapshot
cp /tmp/holyfitra_state2_ast.snapshot "$WORK/state2_ast.first"
(cd "$ROOT" && "$WORK/selfhost_state2")
cmp -s "$WORK/state2_ast.first" /tmp/holyfitra_state2_ast.snapshot
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state2.hf" -o "$WORK/selfhost_state2.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state2.aarch64.ll" -o "$WORK/selfhost_state2.aarch64.o"
test -s "$WORK/selfhost_state2.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_state3.hf" -o "$WORK/selfhost_state3.ll"
clang -O2 "$WORK/selfhost_state3.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state3"
(cd "$ROOT" && "$WORK/selfhost_state3")
test -s /tmp/holyfitra_state3_semantic.snapshot
cp /tmp/holyfitra_state3_semantic.snapshot "$WORK/state3_semantic.first"
(cd "$ROOT" && "$WORK/selfhost_state3")
cmp -s "$WORK/state3_semantic.first" /tmp/holyfitra_state3_semantic.snapshot
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state3.hf" -o "$WORK/selfhost_state3.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state3.aarch64.ll" -o "$WORK/selfhost_state3.aarch64.o"
test -s "$WORK/selfhost_state3.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_state4.hf" -o "$WORK/selfhost_state4.ll"
clang -O2 "$WORK/selfhost_state4.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state4"
(cd "$ROOT" && "$WORK/selfhost_state4")
test -s /tmp/holyfitra_state4_diagnostics.txt
test -s /tmp/holyfitra_state4_diagnostics.json
cp /tmp/holyfitra_state4_diagnostics.txt "$WORK/state4_diagnostics.first.txt"
cp /tmp/holyfitra_state4_diagnostics.json "$WORK/state4_diagnostics.first.json"
(cd "$ROOT" && "$WORK/selfhost_state4")
cmp -s "$WORK/state4_diagnostics.first.txt" /tmp/holyfitra_state4_diagnostics.txt
cmp -s "$WORK/state4_diagnostics.first.json" /tmp/holyfitra_state4_diagnostics.json
clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all "$WORK/selfhost_state4.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state4_san"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/selfhost_state4_san"
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state4.hf" -o "$WORK/selfhost_state4.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state4.aarch64.ll" -o "$WORK/selfhost_state4.aarch64.o"
test -s "$WORK/selfhost_state4.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_state5.hf" -o "$WORK/selfhost_state5.ll"
clang -O2 "$WORK/selfhost_state5.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state5"
(cd "$ROOT" && "$WORK/selfhost_state5")
test -s /tmp/holyfitra_state5_graph.snapshot
cp /tmp/holyfitra_state5_graph.snapshot "$WORK/state5_graph.first"
(cd "$ROOT" && "$WORK/selfhost_state5")
cmp -s "$WORK/state5_graph.first" /tmp/holyfitra_state5_graph.snapshot
clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all "$WORK/selfhost_state5.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state5_san"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/selfhost_state5_san"
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state5.hf" -o "$WORK/selfhost_state5.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state5.aarch64.ll" -o "$WORK/selfhost_state5.aarch64.o"
test -s "$WORK/selfhost_state5.aarch64.o"

rm -rf /tmp/holyfitra_state6_modules
cp -R "$ROOT/bootstrap/state6_modules" /tmp/holyfitra_state6_modules
"$BUILD" "$ROOT/bootstrap/selfhost_state6.hf" -o "$WORK/selfhost_state6.ll"
clang -O2 "$WORK/selfhost_state6.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state6"
(cd "$ROOT" && "$WORK/selfhost_state6")
test -s /tmp/holyfitra_state6_graph.snapshot
cp /tmp/holyfitra_state6_graph.snapshot "$WORK/state6_graph.first"
(cd "$ROOT" && "$WORK/selfhost_state6")
cmp -s "$WORK/state6_graph.first" /tmp/holyfitra_state6_graph.snapshot
clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all "$WORK/selfhost_state6.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state6_san"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/selfhost_state6_san"
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state6.hf" -o "$WORK/selfhost_state6.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state6.aarch64.ll" -o "$WORK/selfhost_state6.aarch64.o"
test -s "$WORK/selfhost_state6.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_state7.hf" -o "$WORK/selfhost_state7.ll"
clang -O2 "$WORK/selfhost_state7.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state7"
(cd "$ROOT" && "$WORK/selfhost_state7")
test -s /tmp/holyfitra_state7_hir.snapshot
cp /tmp/holyfitra_state7_hir.snapshot "$WORK/state7_hir.first"
(cd "$ROOT" && "$WORK/selfhost_state7")
cmp -s "$WORK/state7_hir.first" /tmp/holyfitra_state7_hir.snapshot
clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all "$WORK/selfhost_state7.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state7_san"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/selfhost_state7_san"
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state7.hf" -o "$WORK/selfhost_state7.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state7.aarch64.ll" -o "$WORK/selfhost_state7.aarch64.o"
test -s "$WORK/selfhost_state7.aarch64.o"

"$BUILD" "$ROOT/bootstrap/selfhost_state8.hf" -o "$WORK/selfhost_state8.ll"
clang -O2 "$WORK/selfhost_state8.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state8"
(cd "$ROOT" && "$WORK/selfhost_state8")
test -s /tmp/holyfitra_state8_ast.snapshot
cp /tmp/holyfitra_state8_ast.snapshot "$WORK/state8_ast.first"
test -s /tmp/holyfitra_state8_signatures.snapshot
cp /tmp/holyfitra_state8_signatures.snapshot "$WORK/state8_signatures.first"
test -s /tmp/holyfitra_state8_hir.snapshot
cp /tmp/holyfitra_state8_hir.snapshot "$WORK/state8_hir.first"
(cd "$ROOT" && "$WORK/selfhost_state8")
cmp -s "$WORK/state8_ast.first" /tmp/holyfitra_state8_ast.snapshot
cmp -s "$WORK/state8_signatures.first" /tmp/holyfitra_state8_signatures.snapshot
cmp -s "$WORK/state8_hir.first" /tmp/holyfitra_state8_hir.snapshot
clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all "$WORK/selfhost_state8.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state8_san"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/selfhost_state8_san"
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state8.hf" -o "$WORK/selfhost_state8.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state8.aarch64.ll" -o "$WORK/selfhost_state8.aarch64.o"
test -s "$WORK/selfhost_state8.aarch64.o"

rm -rf /tmp/holyfitra_state9_modules
cp -R "$ROOT/bootstrap/state9_modules" /tmp/holyfitra_state9_modules
"$BUILD" "$ROOT/bootstrap/selfhost_state9.hf" -o "$WORK/selfhost_state9.ll"
clang -O2 "$WORK/selfhost_state9.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state9"
(cd "$ROOT" && "$WORK/selfhost_state9")
test -s /tmp/holyfitra_state9_ast.snapshot
test -s /tmp/holyfitra_state9_signatures.snapshot
test -s /tmp/holyfitra_state9_hir.snapshot
test -s /tmp/holyfitra_state9_children.snapshot
cp /tmp/holyfitra_state9_ast.snapshot "$WORK/state9_ast.first"
cp /tmp/holyfitra_state9_signatures.snapshot "$WORK/state9_signatures.first"
cp /tmp/holyfitra_state9_hir.snapshot "$WORK/state9_hir.first"
cp /tmp/holyfitra_state9_children.snapshot "$WORK/state9_children.first"
(cd "$ROOT" && "$WORK/selfhost_state9")
cmp -s "$WORK/state9_ast.first" /tmp/holyfitra_state9_ast.snapshot
cmp -s "$WORK/state9_signatures.first" /tmp/holyfitra_state9_signatures.snapshot
cmp -s "$WORK/state9_hir.first" /tmp/holyfitra_state9_hir.snapshot
cmp -s "$WORK/state9_children.first" /tmp/holyfitra_state9_children.snapshot
clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all "$WORK/selfhost_state9.ll" "$ROOT/bootstrap/holyfitra_runtime.c" -o "$WORK/selfhost_state9_san"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/selfhost_state9_san"
"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/selfhost_state9.hf" -o "$WORK/selfhost_state9.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/selfhost_state9.aarch64.ll" -o "$WORK/selfhost_state9.aarch64.o"
test -s "$WORK/selfhost_state9.aarch64.o"

clang -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all \
  "$ROOT/bootstrap/holyfitra_runtime.c" "$ROOT/bootstrap/test_holyfitra_runtime.c" \
  -o "$WORK/runtime_san"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "$WORK/runtime_san" >/dev/null

"$BUILD" --target=aarch64-linux-android21 "$ROOT/bootstrap/hello.hf" -o "$WORK/hello.aarch64.ll"
clang --target=aarch64-linux-android21 -c "$WORK/hello.aarch64.ll" -o "$WORK/hello.aarch64.o"
test -s "$WORK/hello.aarch64.o"

if "$BUILD" "$ROOT/bootstrap/invalid_type.hf" >"$WORK/invalid.out" 2>"$WORK/invalid.err"; then
  echo "invalid fixture unexpectedly accepted" >&2
  exit 1
fi
grep -F "error[HF3001]" "$WORK/invalid.err" >/dev/null
grep -F "declared type does not match initializer" "$WORK/invalid.err" >/dev/null
grep -F "|" "$WORK/invalid.err" >/dev/null
grep -F "^" "$WORK/invalid.err" >/dev/null

if "$BUILD" "$ROOT/bootstrap/invalid_syntax.hf" >"$WORK/invalid_syntax.out" 2>"$WORK/invalid_syntax.err"; then
  echo "invalid syntax fixture unexpectedly accepted" >&2
  exit 1
fi
grep -F "error[HF1001]" "$WORK/invalid_syntax.err" >/dev/null
grep -F "|" "$WORK/invalid_syntax.err" >/dev/null
grep -F "^" "$WORK/invalid_syntax.err" >/dev/null

if "$BUILD" "$ROOT/bootstrap/invalid_name.hf" >"$WORK/invalid_name.out" 2>"$WORK/invalid_name.err"; then
  echo "invalid name fixture unexpectedly accepted" >&2
  exit 1
fi
grep -F "error[HF2001]" "$WORK/invalid_name.err" >/dev/null
grep -F "unknown value" "$WORK/invalid_name.err" >/dev/null

# Verify the seed command itself works without Python in PATH or environment.
env -i PATH="$(dirname "$(command -v clang)"):$(dirname "$(command -v clang++)"):/usr/bin:/bin" HOME="$WORK/home" "$BUILD" --help >/dev/null

printf 'bootstrap_host=passed\nbootstrap_short_circuit=passed\nbootstrap_aggregate=passed\nbootstrap_selfhost_frontend=passed\nbootstrap_io=passed\nbootstrap_buffer=passed\nbootstrap_symtable=passed\nbootstrap_typechecker=passed\nbootstrap_emitter=passed\nbootstrap_state1=passed\nbootstrap_state2=passed\nbootstrap_state3=passed\nbootstrap_state4=passed\nbootstrap_state5=passed\nbootstrap_state6=passed\nbootstrap_state7=passed\nbootstrap_state8=passed\nbootstrap_state9=passed\nbootstrap_runtime_sanitizer=passed\nbootstrap_diagnostics=passed\nbootstrap_invalid=passed\nbootstrap_aarch64_object_bytes=%s\nbootstrap_selfhost_aarch64_object_bytes=%s\nbootstrap_buffer_aarch64_object_bytes=%s\nbootstrap_symtable_aarch64_object_bytes=%s\nbootstrap_typechecker_aarch64_object_bytes=%s\nbootstrap_emitter_aarch64_object_bytes=%s\nbootstrap_state1_aarch64_object_bytes=%s\nbootstrap_state2_aarch64_object_bytes=%s\nbootstrap_state3_aarch64_object_bytes=%s\nbootstrap_state4_aarch64_object_bytes=%s\nbootstrap_state5_aarch64_object_bytes=%s\nbootstrap_state6_aarch64_object_bytes=%s\nbootstrap_state7_aarch64_object_bytes=%s\nbootstrap_state8_aarch64_object_bytes=%s
bootstrap_state9_aarch64_object_bytes=%s\nbootstrap_python_free_help=passed\n' "$(stat -c%s "$WORK/hello.aarch64.o")" "$(stat -c%s "$WORK/selfhost_frontend.aarch64.o")" "$(stat -c%s "$WORK/buffer.aarch64.o")" "$(stat -c%s "$WORK/selfhost_symtable.aarch64.o")" "$(stat -c%s "$WORK/selfhost_typechecker.aarch64.o")" "$(stat -c%s "$WORK/selfhost_emitter.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state1.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state2.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state3.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state4.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state5.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state6.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state7.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state8.aarch64.o")" "$(stat -c%s "$WORK/selfhost_state9.aarch64.o")"
