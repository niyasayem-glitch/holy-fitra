...
from holyfitra_deploy import DeploymentBundle, load_deployment_bytes
...
def export_pipeline_capsule(result: PipelineResult, destination: str | os.PathLike[str], *, signing_key: bytes, chunk_bytes: int = 65_536, metadata: dict[str, Any] | None = None, resource_contract: TensorResourceContract | None = None, agent_receipt: AgentPlanReceipt | None = None) -> CapsuleArtifact:
...
def _write_capsule(destination: str | os.PathLike[str], *, deployment: bytes, deployment_hash: str, plan_payload: bytes, receipt_payload: bytes, signing_key: bytes, chunk_bytes: int, metadata: dict[str, Any], resource_contract_payload: bytes | None, agent_receipt_payload: bytes | None) -> CapsuleArtifact:
    key = _validated_key(signing_key)
...
def _validate_manifest(manifest: dict[str, Any], payload_start: int, total_size: int) -> dict[str, CapsuleChunk]:
    ...
    for item in chunks:
        ...
        if name.startswith("deployment/"):
            seen_deployment += 1
        elif name not in {"execution_plan.json", "pipeline_receipt.json", "resource_contract.json", "agent_receipt.json"}:
