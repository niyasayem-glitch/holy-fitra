...
HF now has independently implemented `arg_i32(position, fallback)` and `arg_i64(position, fallback)` builtins for a parameterless `main` that explicitly declares `effects [io]`. They accept only literal positions `0..7`, validate signed decimal argv content with bounded emitted LLVM helpers, and fall back on absent, malformed, trailing-content, or out-of-range input. The i64 route accumulates in i128 to reject the exact signed i64 overflow boundaries before narrowing. Both preserve legacy parameterless-main ABI when unused and expose no general process, file, environment, shell, stdin, or network API. All 40 compiler regressions passed; the i64 boundary test also passed 20 consecutive rounds and existing i32 dynamic input remained compatible. The earlier nine-sample dynamic argv LCG32 host exercise measured whole-process means of 1.811 ms for C and 1.969 ms for HF (1.087×), but it is retained i32 capability evidence rather than an i64-performance result, universal win, or full-width HF numerical proof. Android/ARM/device behavior remains unmeasured. Details are in `HF_DYNAMIC_INPUT_BRIDGE_DESIGN.md` and `HF_DYNAMIC_INPUT_BRIDGE_RESULTS.md`.

## ARM64 i64 runtime gate

The ARM64 runtime gate now has two separate evidence tracks. QEMU 8.2.2 user-mode emulation, the GNU AArch64 sysroot, and a cross Clang link produced an AArch64 ELF from the seven-case HF i64 fixture; all cases passed through QEMU. `validate_hf_i64_qemu_aarch64.sh` records this as emulator-only evidence. Physical ARM64 remains unclosed: no `adb` device or self-hosted ARM runner is available, and GitHub Actions builds arm64-v8a on an x86 Ubuntu runner while explicitly excluding physical-device execution. `validate_hf_i64_arm64.sh` remains the real Termux/ARM64-Linux receipt path. Neither QEMU nor packaging evidence supports CPU, Bionic, JNI, NEON/I8MM, thermal, or device-stability claims; see `HF_ARM64_I64_VALIDATION.md`.
