...
The ARM64 runtime gate was prepared but not closed. The sandbox is x86_64 with no AArch64 emulator or cross-runtime, no connected `adb` device, and no self-hosted ARM runner. GitHub Actions builds arm64-v8a artifacts on an x86 Ubuntu runner and explicitly excludes physical-device execution. `validate_hf_i64_arm64.sh` and its seven-case fixture now provide a Termux/ARM64-Linux receipt path, but its host guard correctly returned `not-run` with exit 77 here. This is a documented blocker, not an ARM64 stability claim; see `HF_ARM64_I64_VALIDATION.md`.
